...
from ipywidgets import interactive_output, VBox, HBox, Checkbox, fixed

def _split_feature(fd1, fd2, fd3, check, sl1, sl2, sl3, df):
    f, (ax, ax1) = plt.subplots(1, 2, figsize=(12, 4))
    ax_ = ax.twinx()

    # Splitting the dataframe based on the threshold sl1 for fd1
    df1 = df.loc[df[fd1] > sl1, :]
    df0 = df.loc[df[fd1] <= sl1, :]
    ms = 30

    # Plot for the first two features
    ax.plot(df0.loc[df0["flood"], fd1], df0.loc[df0["flood"], fd2], 'go', ms=ms, label='Flooded')
    ax.plot(df0.loc[~df0["flood"], fd1], df0.loc[~df0["flood"], fd2], 'ro', ms=ms, label='Not Flooded')
    ax_.plot(df1.loc[df1["flood"], fd1], df1.loc[df1["flood"], fd3], 'go', ms=ms)
    ax_.plot(df1.loc[~df1["flood"], fd1], df1.loc[~df1["flood"], fd3], 'ro', ms=ms)

    # Add legend and labels
    ax.axvline(x=sl1, color="gray", linestyle="--")
    ax.set_xlabel(fd1)
    ax.set_ylabel(fd2)
    ax_.set_ylabel(fd3)

    # Set limits based on check
    if check:
        xlim = ax.get_xlim()
        ax.set_xlim(xlim)
        ax_.set_xlim(xlim)
    ...
    # Second subplot for visualization of decision splits
    ax1.set_xlim([-0.3, 2.5])
    ax1.set_ylim([-3.5, 3.5])
    ax1.set_xticks([])
    ax1.set_yticks([])
    ...
    # Visualize the decision split logic
    ax1.plot([0, 1], [0, 2], 'k-')
    ax1.plot([0, 1], [0, -2], 'k-')
    if check:
        ax1.plot([1, 2], [2, 3], 'k-')
    ...
    # Add text labels for decision points
    if not check:
        c2 = [0.5, 0.5, 0.5]
        c1 = 'k'
    else:
    ...
    # Adjust text positioning to ensure proper alignment for the flood and not-flood columns
    ys = [-2, 2] if not check else [-1, -3, 3, 1]
    dfs = [df0, df1] if not check else [df00, df01, df10, df11]
    
    for y, dfi in zip(ys, dfs):
        s = dfi['flood'].sum()
        us = dfi.shape[0] - s
        ax1.text(1 if not check else 2, y, f'{s:d} Flooded\n{us:d} Not Flooded', bbox=bb, ha='center', color=c2)

    plt.show()


def decision_tree():
    # Synthetic dataset describing seven regions prone to floods.
    # For each, the rainfall, soil property, and saturation level is recorded.
    # An assessment of whether the area has flooded is included as a binary variable.
    data = [
        {"rainfall": 10, "soil_property": "Sandy", "saturation": 90, "flood": True},
        {"rainfall": 60, "soil_property": "Clay", "saturation": 15, "flood": False},
        {"rainfall": 70, "soil_property": "Sandy", "saturation": 75, "flood": True},
        {"rainfall": 20, "soil_property": "Clay", "saturation": 35, "flood": False},
        {"rainfall": 50, "soil_property": "Clay", "saturation": 50, "flood": True},
        {"rainfall": 75, "soil_property": "Sandy", "saturation": 5, "flood": False},
        {"rainfall": 30, "soil_property": "Clay", "saturation": 3, "flood": False},
        {"rainfall": 25, "soil_property": "Clay", "saturation": 65, "flood": False},
        # Additional points
        {"rainfall": 40, "soil_property": "Sandy", "saturation": 80, "flood": True},
        {"rainfall": 35, "soil_property": "Clay", "saturation": 20, "flood": False},
        {"rainfall": 45, "soil_property": "Clay", "saturation": 60, "flood": True},
        {"rainfall": 15, "soil_property": "Sandy", "saturation": 85, "flood": True},
        {"rainfall": 55, "soil_property": "Sandy", "saturation": 45, "flood": True},
        {"rainfall": 65, "soil_property": "Clay", "saturation": 10, "flood": False},
        {"rainfall": 80, "soil_property": "Sandy", "saturation": 25, "flood": False},
        {"rainfall": 30, "soil_property": "Clay", "saturation": 40, "flood": False},
        {"rainfall": 50, "soil_property": "Clay", "saturation": 70, "flood": True},
        {"rainfall": 70, "soil_property": "Clay", "saturation": 55, "flood": True},
        {"rainfall": 20, "soil_property": "Sandy", "saturation": 10, "flood": False},
        {"rainfall": 45, "soil_property": "Clay", "saturation": 65, "flood": True},
        {"rainfall": 35, "soil_property": "Sandy", "saturation": 15, "flood": False},
        {"rainfall": 60, "soil_property": "Sandy", "saturation": 30, "flood": False},
        {"rainfall": 25, "soil_property": "Clay", "saturation": 85, "flood": True},
    ]
    df = pd.DataFrame(data)

    # Create the interactive widgets
    fd1 = widgets.Dropdown(value='rainfall', options=["rainfall", "saturation"], description="Feature:")
    fd2 = widgets.Dropdown(value='saturation', options=["rainfall", "saturation"], description="Feature:")
    fd3 = widgets.Dropdown(value='saturation', options=["rainfall", "saturation"], description="Feature:")
    check = Checkbox(value=False, description="Lock root node")

    # Define sliders for split values
    sl1 = widgets.FloatSlider(
        value=50,
        min=min(df["rainfall"].min(), df["saturation"].min()) - 1,
        max=max(df["rainfall"].max(), df["saturation"].max()) + 1,
        step=1,
        description="Split value:", continuous_update=False)
    sl2 = widgets.FloatSlider(
        value=10,
